...
import { 
  ...
  AlignRight, List, ListOrdered, Type, ChevronDown, Minus, Plus, 
  MoveVertical, Scaling
} from 'lucide-react';
...
const FONTS = [
  'Arial', 'Roboto', 'Times New Roman', 'Georgia', 'Courier New', 
...
export function FloatingTextToolbar({ textItem, onUpdate, position }: FloatingTextToolbarProps) {
  ...
  const toggleDecoration = (type: 'underline' | 'line-through') => {
    ...
    onUpdate({ textDecoration: current === type ? 'none' : type });
  };
  ...
  const handleFontSize = (delta: number) => {
    const newSize = Math.max(8, Math.min(120, (textItem.fontSize || 32) + delta));
  ...
  return (
    <div 
      ...
      className="absolute bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl p-2 flex items-center gap-2 z-50 transition-all duration-200"
    ...
    >
      ...
      <div className="flex items-center gap-1 border-r border-zinc-700 pr-2">
        <div className="relative">
          ...
          {showFontMenu && (
            <div className="absolute top-full left-0 mt-1 w-48 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl max-h-60 overflow-y-auto z-50">
              {FONTS.map(font => (
                <div
                  key={font}
                  onClick={() => {
                  ...
                  className={cn(
                    "px-3 py-2 text-sm text-zinc-300 hover:bg-indigo-600 hover:text-white cursor-pointer transition-colors",
        ...
        <div className="flex items-center bg-zinc-800 rounded-md">
          <button 
            ...
            className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-700 rounded-l-md transition-colors"
          >
            <Minus className="w-3 h-3" />
          </button>
          <span className="w-8 text-center text-xs font-mono text-zinc-200">
            {Math.round(textItem.fontSize || 32)}
          </span>
          <button 
            onClick={() => handleFontSize(1)}
            className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-700 rounded-r-md transition-colors"
          >
      ...
      <div className="relative border-r border-zinc-700 pr-2 flex items-center">
        <button 
          ...
          style={{ backgroundColor: textItem.fill || '#000000' }}
          title="Text Color"
        />
        {showColorMenu && (
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-50">
             ...
             <div className="relative z-50">
               <SketchPicker 
                 ...
                 onChangeComplete={(color) => onUpdate({ fill: color.hex })}
                 styles={{
                   default: {
               ...
               <div className="bg-[#27272a] p-3 border-t border-zinc-700 rounded-b-lg">
                 ...
                 <input 
                    ...
                    className="w-full h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white"
                 />
               </div>
             </div>
          </div>
