...
export default function BookEditor() {
  const { zoom, setZoom, currentPage, setCurrentPage } = useBookStore();
  ...
  const handleExportPDF = async () => {
    setIsExporting(true);
    try {
      await exportBookToPDF(stageRefs, WIDTH, HEIGHT);
    } catch (error) {
  ...
  return (
    <section style={{
      ...
      <div className="max-w-7xl mx-auto flex flex-col h-dvh overflow-hidden">
        <header className="shrink-0 h-13 flex items-center justify-between gap-2 px-5 bg-transparent mb-10">
          <Link href="/">
        ...
        <div className="flex min-h-0 overflow-hidden">
          <div className={
            ...
            (uploadPanelOpen ? "w-52.5 border border-black/6" : "w-0 border-0")
          }>
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0">
            <div className="flex-1 overflow-auto min-h-0">
              <div className="min-h-full flex items-center justify-center px-4 py-5">
                ...
                >
                  ...
                  >
                    <div
                      className="flex bg-white p-12.5 rounded-2xl overflow-hidden shadow-[0_2px_6px_rgba(0,0,0,0.05),0_8px_28px_rgba(0,0,0,0.07)]"
                      style={{ height: CARD_BASE_H }}
                    >
                      <div className="w-[44%] shrink-0 relative bg-[#d9d4cf]">
                        {imgSrc ? (
                          <Image src={imgSrc} alt="Uploaded page image" fill className="object-cover" />
                        ) : (
                          <button
                            type="button"
                            onClick={() => fileInputRef.current?.click()}
                            className="absolute inset-0 w-full h-full flex flex-col items-center justify-center gap-2 bg-linear-to-br from-[#e6e1dc] to-[#d4cec9] cursor-pointer border-0"
                          >
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#bbb" strokeWidth="1.4">
                              <rect x="3" y="3" width="18" height="18" rx="2" />
                              <circle cx="8.5" cy="8.5" r="1.5" />
                              <path d="M21 15l-5-5L5 21" />
                            </svg>
                      ...
                      <div className="flex-1 flex flex-col justify-center px-7 py-7">
                        <h2
                          ...
                          className="font-serif text-[22px] font-bold text-[#1a1a1a] leading-snug mb-3 outline-none"
                        >
                          A Beautiful Journey
                        </h2>
                        <p
                          contentEditable
                          suppressContentEditableWarning
                          className="text-[13px] text-[#b0aba7] leading-[1.75] mb-5 outline-none"
                        >
            ...
            <div className="shrink-0 flex flex-col items-center gap-2 px-4 pt-2 pb-4">
              <div className="flex items-center gap-1.5 bg-white rounded-full px-4 py-1.75 shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-black/4">
                ...
                <PillBtn onClick={() => setZoom(Math.min(150, zoom + 10))} title="Zoom in">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <PillBtn onClick={() => setCurrentPage(Math.max(1, currentPage - 2))} title="Previous spread">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <PillBtn onClick={() => setCurrentPage(Math.min(TOTAL_PAGES - 1, currentPage + 2))} title="Next spread">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="w-full max-w-155 bg-white rounded-2xl px-3 py-3 border border-black/4 shadow-[0_1px_4px_rgba(0,0,0,0.05)] mt-7.5">
                    ...
                    return (
                      ...
                      >
                        {active && !isSingle ? (
                          ...
                          <div
                            className={`w-full max-w-9 mx-auto rounded-[5px] border transition-colors ${
                              active ? "bg-white border-[1.8px] border-[#c0392b]" : "bg-[#e4e1de] border-[#d8d5d2]"
                            }`}
                            style={{ height: 44 }}
                          />
                        )}
