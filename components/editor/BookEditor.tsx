...
// // Categories shown in the Sticker panel (excludes "Suggested/Recent")
// const STICKER_CATS = [
...
// // CSS tweaks injected once to make the picker fit our UI
// const PICKER_CSS = `
//   .EmojiPickerReact {
...
//   // Inject picker CSS once on mount
//   useEffect(() => {
...
//   // Shared handler for both Emoji & Sticker pickers
...
//   // Props shared between both EmojiPicker instances
//   const pickerProps = {
...
//               {/* ── Emoji — full picker with all categories & search ── */}
//               {activePanel === "emoji" && <>
...
//               {/* ── Sticker — same picker, specific categories only ── */}
//               {activePanel === "sticker" && <>
...
//                 style={{ minWidth: `${A4_WIDTH * scale + 64}px`, minHeight: `${A4_HEIGHT * scale + 64}px` }}>
//                 <div
//                   className="shadow-[0_2px_20px_rgba(0,0,0,0.15)] transition-transform duration-200 origin-center"
//                   style={{ width: `${A4_WIDTH}px`, height: `${A4_HEIGHT}px`, transform: `scale(${scale})`, background: page?.background ?? "#ffffff" }}
//                 >
//                   <CanvasPage
//                     pageId={currentPage}
//                     width={A4_WIDTH}
...
//             {/* Element quick-actions (shown only when something is selected) */}
//             {selectedElementId && <>
//               <div className="w-full h-px bg-black/6 my-0.5" />
            ...
            <div className="flex items-center gap-0.5 bg-white/15 rounded-xl p-0.5 border border-white/20">
              {(Object.entries(FORMATS) as [BookFormat, FormatConfig][]).map(([key, config]) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[10px] text-[11px] font-semibold transition-all ${
                    bookFormat === key
