...
export default function QuestionnaireStep({ inviterId, name, email, bookId, questions: inviteQuestions, bookTitle, recipientName, occasion, isAlreadySubmitted, code }: Props) {
    const [answers, setAnswers] = useState<Record<string, string>>({});
    const [photos, setPhotos] = useState<File[]>([]);
    const [photoPreviews, setPhotoPreviews] = useState<string[]>([]);
    const [showDone, setShowDone] = useState(false);
    const [showNoPhotoConfirm, setShowNoPhotoConfirm] = useState(false);
    const [showOnePhotoConfirm, setShowOnePhotoConfirm] = useState(false);
    const fileRef = useRef<HTMLInputElement>(null);
    const { mutateAsync: submitContributionMutation, isPending: isSubmitting } = useSubmitContributionMutation();
    ...
    const doSubmit = async () => {
        try {
            ...
            const response = await submitContributionMutation({ inviterId, code, bookId, payload: formData });

            setShowDone(true);
            const successMsg = response?.message || response?.data?.message || "Your contribution has been sent.";
    ...
    const questionnaireTitle = occasion?.trim() || fetchedOccasion?.trim() || "Birthday";
    return (
            ...
            <main className="max-w-215 mx-auto px-4 sm:px-6 py-7 sm:py-9">
                ...
                <div className="flex items-center gap-2.5 mb-1 mt-6.25">
                    ...
                    <h1 className="text-[20px] sm:text-[22px] font-extrabold text-[#1A1A2E] tracking-tight">
                        Questionnaire <span className="uppercase">{questionnaireTitle}</span>
                    </h1>
                </div>
            ...
            {showOnePhotoConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-5" style={{ background: "rgba(0,0,0,0.35)" }}>
                    <div className="bg-white rounded-2xl w-full max-w-sm px-8 py-8 flex flex-col items-center text-center shadow-[0_24px_64px_rgba(0,0,0,0.18)]">
                        ...
                        <h3 className="text-[17px] font-extrabold text-[#1A1A2E] mb-2">You can still upload 1 more photo</h3>
                        <p className="text-[13px] text-gray-500 mb-6 leading-relaxed">
                            Would you like to add another photo before submitting?
                        </p>
                        <div className="flex gap-3 w-full">
                            <button
                                onClick={() => { setShowOnePhotoConfirm(false); void doSubmit(); }}
                                disabled={isSubmitting}
                                className="flex-1 py-3 rounded-xl border border-gray-200 text-[13px] font-semibold text-gray-600 cursor-pointer hover:bg-gray-50 active:scale-[0.98] transition-all"
                            >
                                {isSubmitting ? "Submitting..." : "No, submit now"}
                            </button>
                            <button
                                onClick={() => { setShowOnePhotoConfirm(false); fileRef.current?.click(); }}
                                className="flex-1 py-3 rounded-xl text-white text-[13px] font-semibold cursor-pointer hover:opacity-90 active:scale-[0.98] transition-all"
                                style={{ background: "linear-gradient(to right, #BF003A, #59001C)" }}
                            >
                                Yes, add photo
                            </button>
