...
//                                     <span className={`text-[11px] whitespace-nowrap mb-2 leading-none
...
//                                             <div className={`h-0.5 mt-7 w-full translate-y-1.5 transition-all duration-300
...
//         if (rows) {
//             gsap.fromTo(rows, { opacity: 0, x: -14 }, { opacity: 1, x: 0, duration: 0.35, stagger: 0.07, ease: "power2.out", delay: 0.3 });
//         }
//     }, []);
...
//         setQuestions(prev => ({
//             ...prev,
//             [subTab]: (prev[subTab] ?? []).map(q => (q.id === id ? { ...q, question: value } : q)),
//         }));
//     };
...
//         if (!e.currentTarget.classList.contains("ring-2")) gsap.to(e.currentTarget, { scale: 1.03, duration: 0.2, ease: "power2.out" });
//     };
...
// // ── Sortable Friend Row (Add Contributors) ────────────────
...
//             ref={setNodeRef}
//             style={style}
//             className={`flex flex-col sm:flex-row gap-2 items-end rounded-xl p-2 transition-colors
//                 ${isDragging
//                     ? "bg-[#fff5f6] border border-[#fcd5de] shadow-lg"
//                     : "border border-transparent hover:border-[#f0edf1] hover:bg-[#fafafa]"
//                 }`}
//         >
//             <button
//                 type="button"
...
//                 <input
//                     value={friend.name}
//                     onChange={e => onUpdate(friend.id, "name", e.target.value)}
...
//                     <input
//                         value={friend.email}
//                         onChange={e => onUpdate(friend.id, "email", e.target.value)}
...
//     const initials = friend.name ? getInitials(friend.name) : "?";
...
//                     {friend.name || "Unnamed contributor"}
//                 </p>
...
//             <div className="w-5 h-5 rounded border border-[#e5e7eb] bg-white shrink-0" />
//         </div>
//     );
// }
...
//     const [pageOrder, setPageOrder] = useState<Friend[]>([]);
...
//     // Sync pageOrder whenever friends change (only named ones)
//     useEffect(() => {
//         const named = friends.filter((f) => f.name.trim() !== "");
//         setPageOrder((prev) => {
//             const prevIds = prev.map((p) => p.id);
//             const newIds = named.map((n) => n.id);
//             const kept = prev.filter((p) => newIds.includes(p.id));
//             const added = named.filter((n) => !prevIds.includes(n.id));
//             const updated = kept.map((k) => named.find((n) => n.id === k.id) ?? k);
//             return [...updated, ...added];
//         });
//     }, [friends]);

//     useEffect(() => {
//         gsap.set([headingRef.current, emailSectionRef.current, friendsSectionRef.current, pageSectionRef.current], { opacity: 0, y: 20 });
//         const tl = gsap.timeline({ defaults: { ease: "power3.out" } });
...
//             .to(pageSectionRef.current, { opacity: 1, y: 0, duration: 0.45 }, "-=0.25");
//     }, []);
...
//     // Page Order DnD
//     const handlePageDragStart = (event: DragStartEvent) => setActivePageId(event.active.id as string);
//     const handlePageDragEnd = (event: DragEndEvent) => {
//         const { active, over } = event;
//         setActivePageId(null);
//         if (!over || active.id === over.id) return;
//         setPageOrder(prev => {
//             const oldIndex = prev.findIndex(f => `page-${f.id}` === active.id);
//             const newIndex = prev.findIndex(f => `page-${f.id}` === over.id);
//             if (oldIndex < 0 || newIndex < 0) return prev;
//             return arrayMove(prev, oldIndex, newIndex);
//         });
//     };
...
//     const activePageFriend = activePageId
//         ? pageOrder.find(f => `page-${f.id}` === activePageId)
//         : null;
...
//                         Customize the invitation email and add the people you&apos;d like to contribute.
//                     </p>
//                 </div>
...
//                     <div className="mb-4">
//                         <label className="text-[13px] font-semibold text-[#374151] block mb-1.5">
//                             Email Subject
//                         </label>
//                         <input
//                             value={emailSubject}
//                             onChange={e => setEmailSubject(e.target.value)}
//                             placeholder="e.g., You're invited to contribute to a memory book!"
...
//                             Use{" "}
//                             <code className="bg-[#f5f5f5] px-1 rounded text-[#374151]">[Name]</code>
//                             {" "}and{" "}
//                             <code className="bg-[#f5f5f5] px-1 rounded text-[#374151]">[Invite Link]</code>
//                             {" "}— they will be replaced automatically for each recipient.
//                         </p>
//                     </div>
//                 </div>

//                 {/* ── Add Contributors Section ── */}
//                 <div ref={friendsSectionRef} className="bg-white rounded-2xl border border-[#f0edf1] p-5 mb-5 shadow-sm">
...
//                     <p className="text-[11px] text-[#9CA3AF] mb-4 mt-1">
//                         Drag contributors with the handle to reorder how their pages appear in the book.
//                     </p>
...
//                                 {/* <span className="text-[12px] font-semibold text-[#374151]">Name</span> */}
//                             </div>
//                             <div className="flex-1">
//                                 {/* <span className="text-[12px] font-semibold text-[#374151]">Email</span> */}
//                             </div>
//                             <div className="w-9 shrink-0" />
//                         </div>
//                     )}
...
//                                     <div className="w-9 h-10.5 rounded-xl border border-[#e5e7eb] bg-[#fafafa] flex items-center justify-center text-[#B91C1C]">
//                                         <DragHandleIcon />
//                                     </div>
//                                     <div className="flex-1 text-[13px] text-[#374151] font-medium truncate">
//                                         {activeFriend.name || "Friend's name"}
//                                     </div>
//                                     <div className="flex-1 text-[13px] text-[#9CA3AF] truncate">
//                                         {activeFriend.email || "friend@email.com"}
//                                     </div>
//                                 </div>
//                             ) : null}
//                         </DragOverlay>
...
//                 {/* ── Contributor Page Order Section ── */}
//                 <div ref={pageSectionRef} className="bg-white rounded-2xl border border-[#f0edf1] p-5 shadow-sm">
//                     <div className="flex items-center gap-2 mb-1">
//                         <div className="w-8 h-8 rounded-lg bg-[#fff5f6] border border-[#fde8ec] flex items-center justify-center shrink-0">
//                             <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#B91C1C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
//                                 <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
...
//                         </div>
//                         <div>
//                             <h2 className="text-[14px] font-bold text-[#1a1a2e]">Contributor page order</h2>
//                             <p className="text-[12px] text-[#9CA3AF]">Drag to reorder how pages appear in the book</p>
//                         </div>
//                     </div>
...
//                             Add contributors above to see the page order here.
//                         </div>
//                     ) : (
//                         <DndContext
//                             sensors={sensors}
...
//                         >
//                             <SortableContext
//                                 items={pageOrder.map(f => `page-${f.id}`)}
//                                 strategy={verticalListSortingStrategy}
//                             >
//                                 <div className="flex flex-col gap-2">
//                                     {pageOrder.map((friend, idx) => (
//                                         <SortablePageRow
//                                             key={`page-${friend.id}`}
//                                             friend={friend}
//                                             index={idx}
...
//                                             className="w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-semibold shrink-0"
//                                             style={{
//                                                 background: avatarColors[pageOrder.findIndex(f => f.id === activePageFriend.id) % avatarColors.length].bg,
//                                                 color: avatarColors[pageOrder.findIndex(f => f.id === activePageFriend.id) % avatarColors.length].text,
//                                             }}
//                                         >
//                                             {getInitials(activePageFriend.name)}
//                                         </div>
//                                         <div className="flex-1 text-[13px] font-semibold text-[#1a1a2e] truncate">
//                                             {activePageFriend.name}
//                                         </div>
//                                     </div>
//                                 ) : null}
//                             </DragOverlay>
...
//                     Your memory book has been created and invitations have been sent to all contributors.
//                 </p>
//                 <Link href="/dashboard">
//                     <button
//                         onClick={handleClose}
...
//             {step === 3 && (
//                 <Step4
//                     onNext={(coverId) => { setSelectedCoverId(coverId); setStep(4); }}
//                     onBack={() => setStep(2)}
//                     initialCoverId={urlCoverId}
//                 />
//             )}
//             {step === 4 && (
//                 <Step2 onNext={() => setStep(5)} onBack={() => setStep(3)} subTab={selectedSubTab} />
//             )}
//             {step === 5 && (
//                 <Step5
//                     onNext={() => setStep(6)}
...
//                             toast.info("Create an account to save and send invites.");
...
function Step3({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
    ...
    const gridRef = useRef<HTMLDivElement>(null);
    const { data: bookPageStylesResponse } = useBookPageStylesQuery();
    const templatesFromApi = bookPageStylesResponse?.data?.map((style) => ({ id: style.id, name: style.name, description: style.description, image: style.image[0] || "/icon/1.jpg" })) ?? [];
    const templates = templatesFromApi.length > 0 ? templatesFromApi : fallbackThemes;
...
function Step4({ onNext, onBack, initialCoverId }: { onNext: (coverId: number) => void; onBack: () => void; initialCoverId?: number }) {
    ...
    const gridRef = useRef<HTMLDivElement>(null);
    const { data: coverPageStylesResponse } = useCoverPageStylesQuery();
    const coversFromApi = coverPageStylesResponse?.data?.map((style) => ({ id: style.id, name: style.name, description: style.description, image: style.image[0] || "/icon/11.jpg" })) ?? [];
    const covers = coversFromApi.length > 0 ? coversFromApi : fallbackCovers;
...
function Step5({ onNext, onBack, coverId, bookDraft }: { onNext: () => void; onBack: () => void; coverId: number; bookDraft: BookDraft | null }) {
    ...
    const previewRef = useRef<HTMLDivElement>(null);
    const { data: coverPageStylesResponse } = useCoverPageStylesQuery();
    const coversFromApi = coverPageStylesResponse?.data?.map((style) => ({ id: style.id, name: style.name, image: style.image[0] || "/icon/11.jpg" })) ?? [];
    const covers = coversFromApi.length > 0 ? coversFromApi : fallbackCovers;
...
function Step6({ onBack, onDone, isSubmitting, errorMessage }: {
    onBack: () => void; onDone: () => Promise<void> | void; isSubmitting?: boolean; errorMessage?: string;
}) {
    const [emailSubject, setEmailSubject] = useState("You're invited to contribute to a memory book! 📖");
    ...
    return (
        <>
            ...
            <BottomNav onBack={onBack} onNext={() => { void onDone(); }} nextLabel="Send Invites" nextDisabled={isSubmitting} />
            {errorMessage && (
                <div className="px-4 sm:px-6 pb-6 max-w-4xl mx-auto w-full">
