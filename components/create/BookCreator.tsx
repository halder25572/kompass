...
// // ✅ FIX #1: Progress bar labels — "Questionnaire" → "Design Questionnaire", "Invite" → "Invite Friends"
...
//     },
//     {
//         // ✅ FIX #1: "Questionnaire" → "Design Questionnaire"
...
// // ✅ FIX #6: "Choose a Book Style" → "Choose a Theme"
...
// // ✅ FIX #5: Accepts initialCoverId to pre-select cover from URL param
...
//                             <Image src={selectedCover.image} alt={selectedCover.name} fill className="object-cover" />
//                         </div>
//                         <div>
...
//                                 <li>Cover: {selectedCover.name}</li>
//                                 <li>Questionnaire Type: {subTab}</li>
//                                 <li>Status: Ready to create</li>
//                             </ul>
//                         </div>
//                     </div>
//                 </div>
//             </div>
//             <BottomNav onBack={onBack} onNext={onNext} nextLabel="Create Book" />
//         </>
//     );
// }
...
//     // ✅ FIX #5: Read cover param from URL — e.g. /create?cover=2
//     // If provided, skip to Choose Cover step (step 3) with that cover pre-selected
//     const urlCoverId = searchParams ? Number(searchParams.get("cover")) || undefined : undefined;

//     useEffect(() => {
//         if (urlCoverId) {
//             setStep(3); // jump straight to Choose Cover step
...
//             {/* ✅ FIX #5: Pass initialCoverId so cover is pre-selected */}
//             {step === 3 && <Step4 onNext={(coverId) => {
//                 setSelectedCoverId(coverId);
//                 setStep(4);
//             }} onBack={() => setStep(2)} initialCoverId={urlCoverId} />}
//             {step === 4 && <Step2 onNext={() => setStep(5)} onBack={() => setStep(3)} subTab={selectedSubTab} />}
//             {step === 5 && <Step5 onNext={() => setShowSuccess(true)} onBack={() => setStep(4)} coverId={selectedCoverId} subTab={selectedSubTab} />}
                    ...
                    {templates.map((tpl) => (
                        <button key={tpl.id} onClick={() => setSelected(tpl.id)}
                            ...
                            className={`tpl-card relative rounded-xl overflow-hidden cursor-pointer group transition-all duration-200
                                ${selected === tpl.id ? "ring-2 ring-[#B91C1C] ring-offset-2" : "ring-1 ring-transparent hover:ring-[#B91C1C]/40"}`}>
                            <div className="relative w-full aspect-4/3 bg-[#d1cfc8]">
                                <Image src={tpl.image} alt={tpl.name} fill className="group-hover:scale-105 transition-transform duration-300 object-cover" />
                                {selected === tpl.id && <CheckIcon />}
                                <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/60 to-transparent px-2 py-2">
                                    <span className="text-white text-[11px] sm:text-[12px] font-medium">{tpl.name}</span>
                                </div>
                    ...
                    {covers.map((cover) => (
                        <button key={cover.id} onClick={() => setSelected(cover.id)}
                            ...
                            className={`cover-card relative rounded-xl overflow-hidden cursor-pointer group transition-all duration-200
                                ${selected === cover.id ? "ring-2 ring-[#B91C1C] ring-offset-2" : "ring-1 ring-transparent hover:ring-[#B91C1C]/40"}`}>
                            <div className="relative w-full aspect-3/4 bg-[#d1cfc8]">
                                <Image src={cover.image} alt={cover.name} fill className="group-hover:scale-105 transition-transform duration-300 object-cover" />
                                {selected === cover.id && <CheckIcon />}
                                <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/60 to-transparent px-2 py-2">
                                    <span className="text-white text-[11px] sm:text-[12px] font-medium">{cover.name}</span>
                                </div>
    ...
    return (
        ...
        >
            ...
            <button
                ...
                {...attributes}
                {...listeners}
                className="mb-[1px] h-[42px] w-9 shrink-0 cursor-grab active:cursor-grabbing rounded-xl border border-[#e5e7eb] bg-white text-[#c5c8cc] hover:text-[#6b7280] hover:border-[#d1d5db] transition-colors flex items-center justify-center touch-none"
            ...
            {canRemove ? (
                <button
                    ...
                    className="mb-[1px] flex items-center justify-center w-9 h-[42px] rounded-xl border border-[#e5e7eb] bg-white text-[#c5c8cc] hover:border-red-200 hover:text-red-400 hover:bg-red-50 transition-colors cursor-pointer shrink-0"
                        ...
                        <DragOverlay dropAnimation={{
                            ...
                            {activeFriend ? (
                                <div className="flex gap-2 items-center rounded-xl border border-[#fcd5de] bg-white shadow-xl px-3 py-2.5 opacity-95">
                                    <div className="w-9 h-[42px] rounded-xl border border-[#e5e7eb] bg-[#fafafa] flex items-center justify-center text-[#B91C1C]">
                                        <DragHandleIcon />
                                    </div>
